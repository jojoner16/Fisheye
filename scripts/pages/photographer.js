...
// dropdown({ currentTarget: classList });
...
function orderWork() {
  ...
  const contentNodes = document.querySelectorAll(".thumb-imgfull");
  ...
  const content = Array.prototype.slice.call(contentNodes);
  ...
  switch (order) {
    ...
    case "date":
      // De + à -
      content.sort(function (item, nextItem) {
        const firstString = item.querySelector("[data-date]").dataset.date;
        const secondString = nextItem.querySelector("[data-date]").dataset.date;
    ...
    case "title":
      // De A à B
      content.sort(function (item, nextItem) {
        const firstString = item
          .querySelector(".thumb-imgfull>:nth-child(2)")
          .textContent.toLowerCase();
        const secondString = nextItem
          .querySelector(".thumb-imgfull>:nth-child(2)")
          .textContent.toLowerCase();
